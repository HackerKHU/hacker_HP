...
| 키 | 무엇 | 수명 |
|---|---|---|
| `photos/uploads/{uuid}.{ext}` | 올렸지만 아직 등록 안 된 원본 | **하루** — 라이프사이클 규칙이 걷어감 |
| `photos/{photoId}/{uuid}.jpg` | 등록된 사진의 리사이즈본 | 사진을 지울 때까지 |
| `photos/{photoId}/thumb/{uuid}.jpg` | 그리드용 썸네일 | 사진을 지울 때까지 |

자료와 같은 이유로 임시 자리를 따로 둔다 — presigned URL로 원본만 올리고 `POST /photos`를 부르지 않는 일이 흔하다. 등록 API가 원본을 읽어 리사이즈한 뒤 최종 위치에 다시 쓰고 임시 원본을 지운다(`PhotoService#registerOne`) — **버킷 이름은 자료와 같은 `S3_BUCKET`**을 쓴다. 물리적으로 같은 버킷이라서다(위 표의 접두사로만 구분한다).
