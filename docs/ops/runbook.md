...
**한 계정이 15분에 5회 실패하면 그 계정은 같은 창 동안 막힙니다.** 이 경로 전체가 15분에 20회 실패해도 막힙니다 ([3-2 §3-2-3](../../spec/3-2-DESIGN-CONTRACT.md), #144).
...
**해제 수단은 없습니다. 15분을 기다리세요.** 해제 스위치는 그 자체가 새 공격 표면이고, RDS가 프라이빗이라 DB에서 직접 지우는 것은 15분보다 오래 걸립니다.

**성공하면 그 계정의 실패 기록은 지워집니다.** 토큰을 잘못 붙여넣어 몇 번 틀린 뒤 성공했다면, 다음 사고 때 그것 때문에 막히지 않습니다.
