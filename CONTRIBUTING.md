...
- subject는 **명사형("~추가")과 서술형("~한다") 둘 다 좋습니다.** 한 제목 안에서 섞지만 않으면 됩니다. 무엇을 하는 커밋인지가 드러나는 쪽을 고르세요 — 짧은 변경은 명사형이, 판단이 담긴 변경은 서술형이 읽기 쉽습니다.
- 본문이 필요하면 빈 줄 하나 띄우고 **왜** 바꿨는지 위주로 씁니다. 무엇을 바꿨는지는 diff가 보여줍니다.
- 제목은 **72자** 안에서 씁니다. squash merge가 ` (#123)`을 덧붙이므로 그만큼 여유가 필요합니다.
- 커밋 제목에 이슈 번호를 넣지 않습니다. 이슈 연결은 브랜치명과 PR 본문이 담당하고, 최종 히스토리의 번호는 squash merge가 붙여 줍니다.

### 형식을 검사하는 것

- **관문은 CI입니다.** squash merge에서 히스토리에 남는 것은 PR 제목이므로, `Lint PR title` 워크플로가 그것을 검사합니다. **커밋과 PR 제목이 같은 규칙을 씁니다** — 훅을 통과한 제목이 CI에서 막히거나 그 반대이면 규칙이 둘로 갈린 것입니다.
- **로컬 훅은 편의입니다.** 저장소 루트에서 `npm install`을 한 번 하면 `commit-msg` 훅이 설치되어 커밋할 때 같은 규칙으로 검사합니다. **설치하지 않아도 아무것도 막히지 않습니다** — 작업 브랜치의 커밋은 스쿼시되어 사라지기 때문입니다.

```bash
npm install   # 저장소 루트에서 한 번
```

규칙의 원본은 이 문서이고, `commitlint.config.mjs`가 그것을 기계가 읽는 형태로 옮긴 것입니다. **둘이 다른 말을 하면 안 됩니다** — 도구가 문서에 없는 것을 막으면 사람은 도구를 끕니다.
