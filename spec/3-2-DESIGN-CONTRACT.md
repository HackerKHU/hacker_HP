...
§3-2-7   공통 에러 코드
§3-2-8   공통 페이지 응답
§3-2-9   미합의 — JSON 필드명, 성공 상태 코드, 일괄 승인 응답
```
...
**이 절은 결정이 아니라 확인이 필요한 항목이다.** 계약이 말하지 않는데 프론트엔드가 이미 가정하고 구현해 굳어버린 둘을 사실 그대로 적는다. 백엔드 담당과 맞춘 뒤에 이 절을 지우고 해당 절(§3-2-2, §3-2-5 등)에 확정 내용을 넣는다. 합의 전까지는 **어느 쪽이 맞다고 읽지 않는다.**
