...
| 없는 자료에 `POST` | `404 NOT_FOUND` — 그대로 넣으면 FK 위반이 `500`이 된다 |
...
**`GET /bookmarks` 응답은 `GET /notes`와 같은 형태다.** 같은 카드를 그리는 화면이라 응답이 다르면 화면이 두 벌이 된다. 그 목록의 `bookmarked`는 언제나 `true`다.
