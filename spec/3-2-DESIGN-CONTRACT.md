...
`POST /admin/users/approve`의 응답은 아래로 확정한다 (2026-08-13, #30). 프론트엔드가 먼저 구현하고 §3-2-9에 확인을 요청했던 형태를 그대로 받았다.
...
**활성 관리자 수 확인과 변경은 한 연산이다** (§2-2-7 MUST). 활성 관리자 행을 잠근 채 세므로, 동시에 들어온 다른 정지는 그 잠금을 기다렸다가 줄어든 수를 보고 막힌다 — 각자 자기 자신을 정지하든 **서로를 정지하든** 마찬가지다 ([5-TESTING T-15](5-TESTING.md#5-2-필수-테스트-사례)).
