...
| `action` | enum | NOT NULL | `APPROVE`, `SUSPEND`, `ACTIVATE`, `DEACTIVATE`, `REACTIVATE`, `PROMOTE_ADMIN` |
| `created_at` | datetime | NOT NULL | |
...
**3번이 사람의 조작이라 이 순서가 성립한다** (MUST). 배포만으로는 어떤 계정도 새 값을 갖지 않으므로, 1과 2 사이에 낡은 번들이 새 값을 받을 창이 아예 없다. **사람의 조작 없이 값이 생기는 상태를 더한다면 이 순서로는 부족하고, 그때는 `v2`를 검토한다.**
...
**`deactivated`는 실제로 바뀐 id다** (MUST). 조건으로 실행했으므로 **이 배열이 아니면 무엇이 바뀌었는지 알 방법이 없다.** 잘못 눌렀으면 이 배열을 그대로 복구 요청에 넣는다. 이미 `INACTIVE`였던 사람은 여기 들어가지 않는다.
...
**그 `500`은 `deactivated`를 함께 담는다** (MUST, 2026-08-26 리뷰).

```json
500  { "code": "INTERNAL_ERROR", "message": "...", "deactivated": [1, 2, 7] }
```

**오류 본문에 필드를 더하는 유일한 경로다.** 담지 않으면 **되돌릴 입력이 그 자리에서 사라진다** — 상태 변경은 이미 커밋됐고, 같은 요청을 다시 보내면 전원이 이미 `INACTIVE`라 `{"deactivated": []}`가 돌아온다. 관리자는 **원래 비활동이던 사람과 방금 내려간 사람을 가를 수 없고**, 유일한 되돌리는 길인 복구에 넣을 명단을 만들지 못한다. 재요청은 세션을 다시 반영해 주지만 명단을 되찾아 주지는 않는다.

**응답 자체를 놓쳤을 때의 원본은 이력이다.** `DEACTIVATE` 행은 상태 변경과 **같은 트랜잭션에서** 커밋된다 (MUST) — 세션 반영은 그 뒤라, 반영이 실패해도 이력은 남는다. 브라우저가 닫히거나 연결이 끊겨 응답을 아예 못 받았으면 그 배치를 이력에서 읽는다 ([2-2 §2-2-7](2-2-OPERATOR-REQUIREMENTS.md#2-2-7-안전장치)). **이력을 나중에 쌓는 구현은 이 최후 수단을 없앤다.**
