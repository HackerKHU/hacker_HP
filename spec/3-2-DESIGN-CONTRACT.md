...
**뒤 두 글자를 그대로 쓴다** (MUST). 신규 신청은 숫자만 받지만([§3-2-3](#3-2-3-api--인증)),
그 규칙 전에 저장된 비숫자 학번은 소급 변경하지 않는다. **여기서 하려는 일은 구별이지 학번
해석이 아니다.** 표시 단계가 숫자 규칙을 다시 강제하면 기존 계정만 이름이 달라진다.

**"두 글자"는 코드포인트 기준이다** (MUST). Java의 `substring(length - 2)`는 **UTF-16
코드 유닛**으로 자르므로, BMP 밖 문자(이모지 등)가 걸리면 **서로게이트 쌍을 반으로 쪼개**
깨진 글자를 내보낸다. 신규 신청 규칙을 소급하지 않으므로 기존 값에는 그런 문자가 남을 수
있다 (T-434).

> 신규 신청을 숫자로 제한해도 **이 규칙은 그대로다** — 이미 저장된 값은 소급해 바꾸지 않으므로 표시 규칙이 기존 비숫자 값도 견뎌야 한다.
...
**`studentNo`는 숫자만 받는다** (MUST, 2026-08-31 [#328](https://github.com/HackerKHU/hacker_HP/issues/328)). `[0-9]`만 허용하고, 아니면 `400 VALIDATION_ERROR`다. **`applied_at`은 여기서도 기록하지 않는다.**

> 한때는 형식을 걸지 않았다 — *"편입·교환학생·대학원처럼 형태가 다른 학번이 실제로 있다"* 는 이유였다 ([#38](https://github.com/HackerKHU/hacker_HP/issues/38)). **그 전제를 확인한 결과 학교 학번은 전부 숫자였다.** 근거가 사라졌으므로 규칙을 세운다. 형식이 없으면 `"편입2025"`·`"😀A"` 같은 값이 저장되고, 그것이 **표시 이름의 학번 뒷자리로 화면에 나간다** ([§3-2-2](#표시-이름은-이름--학번-끝-두-자리다-2026-08-30-확정-300)).

- **자릿수는 고정하지 않는다.** 1자든 20자든 숫자면 받는다. 폼 예시가 10자리지만 **그것이 실제 자릿수라는 근거가 없고**, 길이를 규칙으로 굳히면 그 길이가 아닌 학번을 가진 사람이 가입하지 못한다 — 승인 뒤에는 정정할 경로도 없다 ([#178](https://github.com/HackerKHU/hacker_HP/issues/178)).
- **`[0-9]`로 적는다. `\d`가 아니다** (MUST). Java의 `\d`에 유니코드 플래그가 붙으면 아라비아-인도 숫자(`٢٠٢٤`)까지 통과하고, 그 글자가 표시 이름에 그대로 나간다.
- **공백 제거가 검증보다 먼저다** (MUST). `"2024 0001"`은 안쪽 공백이 지워져 통과한다 — 순서가 뒤집히면 **정상 학번이 형식 위반으로 거절된다.**
- **Web은 raw `maxLength`를 걸지 않는다** (MUST). 공백 제거 뒤 20자리면 raw 입력이 20자를
  넘어도 서버가 받는다. 화면도 같은 값을 정규화한 뒤 숫자 여부와 20자 상한을 검사한다.
- **빈 값과 숫자 아님은 같은 문구로 답한다** (MUST). 오류 응답은 실패한 것 중 하나만 골라 내보내고 그 순서가 정해져 있지 않아, 문구가 다르면 **같은 입력에 다른 안내가 나간다.**
- **이미 저장된 값에는 소급하지 않는다.** 이 규칙은 신청 경로에만 걸린다 — 소급하면 그 계정은 재신청도 하지 못한다.
...
**`GET /posts` 응답** — 한 행은 `id`·`title`·`author`·`createdAt`·`likeCount`·`likedByMe`·`commentCount`다.

**`commentCount`는 목록에만 있다** (#374, [3-3 결정 30](3-3-DESIGN-DECISIONS.md#3-3-31-결정-30--게시글-댓글-수는-세어서-목록-응답에-담는다)). 목록에서 "댓글이 달린 글"을 가르는 값이라 여기 있어야 한다 — 글마다 따로 묻는 경로를 두면 한 페이지에 요청이 스무 번 나가고 숫자가 행마다 늦게 채워진다. **상세에는 담지 않는다** — 그쪽은 `GET /posts/{postId}/comments`가 댓글을 통째로 내려주므로 화면이 배열 길이로 안다.

**세는 값이지 저장하는 값이 아니다** (MUST). `posts`에 카운터 열을 두지 않는다 — 댓글 등록·삭제와 게시글 삭제 CASCADE마다 그 열을 따로 맞춰야 하고 한 번 어긋나면 되돌릴 근거가 없다. 좋아요가 같은 갈림길에서 카운터를 버린 것과 같은 판단이다([결정 24](3-3-DESIGN-DECISIONS.md#3-3-25-결정-24--공지사항에-좋아요를-더한다) D1). 그래서 **댓글이 지워지면 수가 곧바로 따라 준다.** 댓글이 없는 글은 `0`이며 필드가 빠지거나 `null`이 되지 않는다.
