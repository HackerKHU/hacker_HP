...
여기서 말하는 것은 **승인의 대상 집합**이지 회원 목록 화면이 아니다 (2026-08-10 명확화). `GET /admin/users`는 모든 회원을 보여주는 화면이고 신청하지 않은 계정도 관리자가 봐야 한다 — 그 사람이 존재한다는 것 자체가 운영 정보다. 요점은 **그 계정이 승인 대상에서 빠지는 것**이고, 화면은 이를 상태 표시("신청 전")와 선택 불가로 드러낸다 ([5-TESTING T-73](5-TESTING.md#5-2-필수-테스트-사례)).
...
그때까지의 임시 조치: 화면은 상태 필터의 `PENDING` 라벨을 **"미승인"**으로 쓴다. 신청 전과 승인 대기를 **둘 다** 데려오므로 둘을 아우르는 이름이어야 한다 — "승인 대기"라고 적으면 필터가 거짓말을 한다. 목록의 상태 칸은 여전히 "미승인"과 "승인 대기"를 가른다.
