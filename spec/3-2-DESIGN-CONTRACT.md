...
**전원 경로는 기존 응답 `{"deactivated": [...]}`를 정확히 유지하고 `failed` 필드를 내리지 않는다** (MUST). 선택 경로만 `failed`를 항상 배열로 돌려준다. 일부 실패도 `200`이고 성공한 변경은 되돌리지 않는다.

| `reason` | 상황 |
|---|---|
| `NOT_FOUND` | 그 id의 계정이 없다 |
| `NOT_ACTIVE_USER` | `role=USER AND status=ACTIVE`가 아니다 — `ADMIN`·`SUSPENDED`·`PENDING`·이미 `INACTIVE` 등을 한 안정된 사유로 묶는다 |
...
**세션 반영은 변경 대상보다 넓다** (MUST). 전원 경로는 `ACTIVE`·`INACTIVE` 일반 부원 전원, 선택 경로는 선택 id 중 두 상태인 일반 부원이 대상이다. 이미 내려간 선택 회원도 같은 요청의 재시도에 포함하되 선택하지 않은 회원은 건드리지 않는다. **반영에 실패하면 `500 INTERNAL_ERROR`이고 변경은 되돌리지 않는다** ([2-2 §2-2-5](2-2-OPERATOR-REQUIREMENTS.md#차단이-강해지는-변경은-세션에-닿아야-성공이다) MUST).
