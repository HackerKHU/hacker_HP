...
| T-73 | 신청서를 내지 않은 `PENDING`(`applied_at IS NULL`) 행 | 체크박스를 선택할 수 없고 **왜 안 되는지가 화면에 보인다** |
...
| T-77 | 승인 버튼을 누른 직후 (확인 전) | 승인 요청이 나가지 않는다. 확인 창에 **승인할 사람의 이름**이 보인다 |
...
T-73~T-76이 한 벌이다. 승인 대상은 `status = PENDING AND applied_at IS NOT NULL`이고([3-2 §3-2-6](3-2-DESIGN-CONTRACT.md) MUST), 신청하지 않은 계정을 승인하면 **학번이 빈 `ACTIVE`가 만들어진다.** 서버가 거르지만 화면도 같은 조건으로 잠근다 — 다만 잠그기만 하고 이유를 말하지 않으면 관리자는 고장난 줄 안다(T-73).
...
T-80은 화면이 **판단하지 않는 것**을 확인한다. 활성 관리자가 몇 명인지 화면은 모르므로 미리 비활성화하지 않고([2-2 §2-2-7](2-2-OPERATOR-REQUIREMENTS.md) — 검사는 서버에서 한다), 서버가 403으로 막으면 그 사유를 그대로 보여준다. T-79와 같은 뿌리다 — 막힌 것을 성공처럼 보이게 하면 관리자가 정지된 줄 알고 자리를 뜬다.
