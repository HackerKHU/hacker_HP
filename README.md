...
동아리 구성원이 공지와 학습 자료를 공유하는 승인제 내부 웹사이트다. **[khuhacker.com](https://khuhacker.com)으로 운영 중이며 `main` 브랜치가 배포된다.** 1차 출시는 공개 랜딩·인증·회원 관리·공지였고, 아래 `Post Launch` 기능도 상당수 들어갔다. 공개된 회원 콘텐츠는 랜딩뿐이며 정적으로 구현한다 — 로그인·회원가입 신청 화면은 원래 비로그인용이라 이 제한의 대상이 아니다 ([spec/3-3 결정 8](spec/3-3-DESIGN-DECISIONS.md#3-3-9-결정-8--공개-랜딩-페이지를-1차-출시에-포함하고-정적으로-구현한다)).
...
범위 변경의 원본은 [배경과 1차 출시 범위](spec/1-BACKGROUND.md#1-6-1차-출시-범위), 결정 이유는 [결정 7](spec/3-3-DESIGN-DECISIONS.md#3-3-8-결정-7--1차-출시를-인증회원-관리공지로-제한한다)과 [결정 8](spec/3-3-DESIGN-DECISIONS.md#3-3-9-결정-8--공개-랜딩-페이지를-1차-출시에-포함하고-정적으로-구현한다)이다.
...
**MVP는 배포되어 운영 중이고**, `Post Launch`의 자료·활동사진·회원 제거·권한 변경도 들어가 있다. 새 기능에 착수하기 전에 [미결정 사항](spec/1-BACKGROUND.md#1-5-미결정-사항) 중 그 기능의 선행 결정을 닫고, 변경된 권한·스키마·API는 코드와 같은 PR에서 문서화한다.
