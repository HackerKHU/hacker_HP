...
동아리 구성원이 공지와 학습 자료를 공유하는 승인제 내부 웹사이트다. 현재 저장소는 스펙이 확정된 구현 착수 단계이며, 1차 출시는 인증·회원 관리·공지에 집중한다.
...
| 승원 | 인증·권한·회원 관리 백엔드 |
| Codex | PR 보조 리뷰 |
| 팀 전체 | 사람 리뷰, 통합 검증, 배포 리허설 |

수민과 승원은 맡은 백엔드 도메인을 병렬로 진행하고 서로의 PR을 리뷰한다. Codex 리뷰는 사람 승인을 대체하지 않는다.
