...
동아리 구성원이 공지와 학습 자료를 공유하는 승인제 내부 웹사이트다. 현재 저장소는 스펙이 확정된 구현 착수 단계이며, 1차 출시는 공개 랜딩·인증·회원 관리·공지에 집중한다. 공개된 회원 콘텐츠는 랜딩뿐이며 정적으로 구현한다 — 로그인·회원가입 신청 화면은 원래 비로그인용이라 이 제한의 대상이 아니다 ([spec/3-3 결정 8](spec/3-3-DESIGN-DECISIONS.md#3-3-9-결정-8--공개-랜딩-페이지를-1차-출시에-포함하고-정적으로-구현한다)).
