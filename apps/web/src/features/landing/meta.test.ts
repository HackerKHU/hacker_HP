...
describe('index.html 메타 태그', () => {
  ...
  /*
   * ⚠️ **지금 실패하는 단언이라 꺼 두었다. 파일이 준비되면 `.skip`을 떼면 통과한다.**
   *
   * `index.html`의 `og:image`는 `/landing/og-image.png`를 가리키는데 그 파일이 아직 없다
   * (`public/landing/README.md`가 이미 없다고 적어둔 항목이다). 태그 존재 검사만으로는
   * 이 상태가 green이라, 미리보기 이미지가 404인 채로 T-62를 만족한다.
   *
   * OG 이미지 준비 여부는 콘텐츠 결정이라 여기서 임의로 처리하지 않는다 — 이미지를 넣든,
   * `og:image` 태그를 뺐다가 나중에 되돌리든 결정된 뒤에 이 줄을 켠다.
   */
  it.skip('og:image가 가리키는 파일이 실제로 있다', () => {
