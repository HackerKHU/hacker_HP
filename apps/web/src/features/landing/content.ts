...
export const CLUB = {
   ...
   *
   * TODO: 실제 모집 폼 주소로 교체. 모집은 상시로 열려 있지만 폼 주소는 바뀔 수 있다 —
   * 인스타그램 프로필의 링크와 어긋나면 갱신한다.
   *
...
interface Stat {
  ...
  minDigits?: number
  /**
   * 정확한 집계가 아니라 근사치인지. 숫자 뒤에 `+`를 붙여 "이만큼 이상"으로 읽히게 한다.
   *
   * 값은 `number`로 남기고 표기 여부만 따로 둔다. `'2000+'`처럼 문자열로 넣으면
   * 카운트업이 숫자를 셀 수 없고, 위 `value` 주석이 경고하는 파싱 문제로 되돌아간다.
   */
  approx?: boolean
  label: string
}
...
export const STATS: Stat[] = [
  ...
  // ⚠️ 학기마다 바뀌는 값이다. 정식 등록 인원 기준이며, 학기가 바뀌면 갱신한다.
  //    2026학년도 1학기 기준.
  { value: 65, unit: '명', label: '현재 활동 회원' },
  // 1987년 창립 이후 39년을 기준으로 잡은 근사치다. 정확한 명부 집계가 아니므로
  // `approx`로 `+`를 붙여 "이만큼 이상"으로 읽히게 한다. 정확한 수처럼 보이면 안 된다.
  { value: 2000, unit: '명', approx: true, label: '누적 활동 회원' },
]
...
export const FAQS: Faq[] = [
  ...
  },
  {
    question: '모집은 언제 하나요?',
    answer:
      '상시 모집합니다. 지원 폼은 언제든 열려 있고, 소식은 공식 인스타그램으로 알려드립니다.',
  },
]
