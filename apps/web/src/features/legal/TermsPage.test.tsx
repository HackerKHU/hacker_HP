...
describe('이용약관', () => {
  ...
  /*
   * **약관이 권한 매트릭스와 정확히 같아야 한다** (3-1 §3-1-3). 작성자에게는 닫고
   * 운영진에게만 연 결정 20의 경계를 그대로 알린다.
   */
  it('게시판 글은 작성자가 직접 지울 수 없고 운영진이 완전 삭제한다고 밝힌다', async () => {
    await openTerms()

    expect(
      screen.getByText(/올리신 분이 직접 지우실 수 없지만/),
    ).toBeInTheDocument()
    expect(
      screen.getByText(/운영진은 관리 화면에서 완전히 삭제/),
    ).toBeInTheDocument()
  })
