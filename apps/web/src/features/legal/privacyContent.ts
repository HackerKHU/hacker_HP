 ...
 *
 * ⚠️ **DB 스키마가 아직 이 문서를 따라오지 못했다.** 수집 항목을 "구글 계정에서 받는
 * 이메일·이름"으로, 비밀번호를 저장하지 않는 것으로 쓴 것은 [3-3 결정 13]을 따른 것이고
 * 그 결정과 계약(§3-2 `users` 표)은 PR #77로 `develop`에 들어와 있다.
 *
 * 그런데 **`V1__init.sql`에는 여전히 `password_hash NOT NULL`이 있고 `google_sub`·
 * `applied_at`이 없다.** 마이그레이션이 갱신되기 전에는 이 문서가 실제 저장 내용과
 * 어긋난다 — **구현이 계약을 따라잡은 뒤에 공개한다.**
 *
