...
describe('구글 버튼', () => {
   ...
   * | 이 파일이 잡는다 | 잡지 못한다 |
   * |---|---|
   * | 마크업의 구조·속성 이름·속성 값(좌표·색·필터 값·그라디언트 정지점) | **부모 요소에 건 `filter`·`opacity`** |
   * | 바깥 `<svg>`의 `viewBox`와 `fill` | `mask`·`clip-path`로 일부를 가리는 것 |
   * | **로고**에 건 것 — 인라인 `style`과 **클래스 전부** | 가상 요소(`::before`/`::after`) 오버레이 |
   * | **버튼**에 건 인라인 `style` | **버튼 클래스 중 이름으로 못 거르는 것** — 아래 |
   * | 버튼 클래스 중 **이름이 알려진 유틸리티** — `opacity-*`·`blur-*`·`grayscale-*`·`mix-blend-*` 등. 상태 접두사(`hover:` …)가 붙어도 걸리고 `disabled:`만 뺀다 | |
   *
   * **로고와 버튼의 검사 방식이 다르다.** 로고는 클래스가 `size-*` 하나뿐이라 허용 목록을
   * 쓸 수 있어 **어떤 형태든** 걸린다. 버튼은 규격·레이아웃 클래스가 스무 개 넘어 허용
   * 목록을 만들 자리가 아니라 **이름으로 거른다** — 그래서 이름이 다르면 지나간다.
   * 실제로 돌려 확인한 것:
   *
   * | 버튼에 건 클래스 | |
   * |---|---|
   * | `opacity-50`, `hover:opacity-90`, `blur-sm`, `mix-blend-multiply`, `brightness-50` | 걸린다 |
   * | `grayscale` (대시 없는 표준 유틸리티) | **지나간다** |
   * | `filter-[grayscale(1)]` (arbitrary value) | **지나간다** |
   * | `[filter:grayscale(1)]`, `[opacity:0.5]` (임의 속성) | **지나간다** |
   *
   * **오른쪽 칸을 막지 않는 것은 의도다.** CSS로 그림을 바꾸는 경로는 열거할 수 없어서,
   * 하나를 막으면 다른 하나가 나오고 목록이 길어질수록 정당한 변경까지 막을 위험만 커진다.
   * 대신 오른쪽 칸은 전부 **의도가 있어야 들어오는 형태**다 — 실수로 부모에 `filter`가
   * 붙거나, 버튼에 `[filter:...]`를 적거나, 가상 요소 오버레이가 생기지는 않는다.
   *
