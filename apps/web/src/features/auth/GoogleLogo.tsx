 ...
 * 구글 **공식** G 로고.
 *
 * `signin-assets.zip`의 `Android + Web/SVG/Light/Theme=Light, Show text=No, Shape=Square`
 * 에서 **로고만 뽑았다.** 걷어낸 것은 그 파일의 40×40 버튼 껍데기(흰 배경 + `#747775`
 * 테두리) 두 줄뿐이고, 나머지는 한 글자도 고치지 않았다. 좌표계도 원본 그대로라
 * `viewBox`만 로고 부분(`10 10 20 20`)으로 잘라 쓴다.
 *
 * **직접 그리거나 색을 바꾸지 않는다** (브랜딩 가이드라인 MUST — 자체 제작 아이콘 금지,
 * 색 변경 금지, 단색 금지). 2025년 개편판이라 네 가지 단색이 아니라 원뿔 그라디언트와
 * 흐림 타원으로 그려져 있다.
 *
 * `aria-hidden` — 버튼의 이름은 옆의 한글 텍스트가 담당한다.
 */

/**
 * 공식 마크업. **JSX로 옮기지 않고 문자열로 둔다.**
 *
 * 이 그림의 색은 `<foreignObject>` 안의 HTML `<div>`가 그리는 CSS 원뿔 그라디언트다.
 * React가 만드는 엘리먼트는 `foreignObject` 안에서도 **SVG 네임스페이스**라 그 `<div>`가
 * HTML div가 아니게 되고, 배경이 칠해지지 않아 로고가 흐릿한 얼룩으로 나온다 —
 * 실제로 그렇게 렌더됐다. HTML 파서를 태우면 `foreignObject`가 네임스페이스 전환점이라
 * 제대로 만들어진다.
 *
 * 사용자 입력이 섞이지 않는 **고정 문자열**이다.
 */
const OFFICIAL_MARKUP = `<mask id="mask0_1298_12516" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="10" y="10" width="20" height="20">
...
export function GoogleLogo({ className }: { className?: string }) {
  return (
    <svg
      ...
      focusable="false"
      className={className}
      // biome-ignore lint/security/noDangerouslySetInnerHtml: 위 주석 참조 — 공식 마크업을 파서에 태워야 foreignObject가 HTML 네임스페이스로 만들어진다. 사용자 입력이 없는 고정 문자열이다.
      dangerouslySetInnerHTML={{ __html: OFFICIAL_MARKUP }}
    />
  )
}
