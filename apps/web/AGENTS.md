...
- **목록을 거르는 일은 서버가 한다.** 받아서 화면에서 버리면 총 건수와 총 페이지 수가 실제와 어긋난다 — 관리자가 읽는 숫자가 틀리게 된다. 서버에 파라미터가 없으면 이슈로 올린다.
- **조회 조건은 URL에 둔다.** 뒤로가기·새로고침·링크 공유에 살아남아야 한다. **서버 파라미터를 그대로 적는다** — 합성 값을 쓰면 주소만 보고는 무엇을 조회하는지 알 수 없다.
- **필터의 낱말은 그 필터가 실제로 데려오는 범위와 같아야 한다.** 같은 낱말이 두 곳에서 다른 범위를 뜻하면 화면이 관리자를 속인다.
