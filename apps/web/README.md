...
| **넓은 화면** (`<main>` 1152px — 공지 작성·수정) | **오른쪽 정렬**, `flex justify-end gap-2`에 `[취소] [저장]` | 1152px짜리 버튼은 우스워진다 |
...
| 공지 목록·상세·작성·수정, 회원 관리 | 위에서 시작 | 가운데로 두면 **행 수나 글 길이에 따라 시작 위치가 위아래로 움직인다.** 목록은 첫 행이 늘 같은 자리에 있어야 하고, 공지 상세는 글마다 제목 위치가 달라지면 안 된다 |
