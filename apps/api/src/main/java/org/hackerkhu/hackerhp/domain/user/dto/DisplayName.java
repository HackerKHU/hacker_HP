...
public final class DisplayName {
   ...
   *
   * <p><b>숫자인지 따지지 않는다.</b> 신규 신청은 숫자만 받지만(3-2 §3-2-3), 그 규칙 전에 저장된 비숫자 학번은 소급 변경하지 않는다. <b>여기서 하려는
   * 일은 구별이지 학번 해석이 아니다.</b> 표시 단계가 새 신청 규칙을 다시 강제하면 기존 계정만 이름이 달라진다.
   *
   * <p><b>코드포인트로 센다</b> (MUST, T-434). {@code substring(length - 2)}는 UTF-16 코드 유닛을 자르므로, BMP 밖
   * 문자(이모지 등)가 걸리면 <b>서로게이트 쌍을 반으로 쪼개</b> 깨진 글자를 내보낸다. 신규 신청 규칙을 소급하지 않으므로 기존 값에는 그런 문자가 남을 수 있다.
   */
