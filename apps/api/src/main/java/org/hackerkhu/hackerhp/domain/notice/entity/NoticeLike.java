...
/**
 * spec/3-2-DESIGN-CONTRACT.md §3-2-2 {@code notice_likes} (#343).
 *
 * <p><b>회원·공지를 연관관계로 매핑하지 않는다.</b> 양쪽 FK가 {@code ON DELETE CASCADE}라 주인이 사라지면 이 행도 함께 사라진다 — {@code
 * Bookmark}와 같은 판단이다. 이 표가 답하는 것은 <b>"그 조합이 있나"</b>뿐이다.
 *
 * <p>고쳐 쓰지 않는다. 좋아요는 있거나 없거나다.
 *
 * <p><b>이 엔티티로 새로 만들지 않는다.</b> 담기·빼기는 {@code NoticeLikeRepository}의 한 문장짜리 질의가 한다 — 확인하고 저장하는 방식은
