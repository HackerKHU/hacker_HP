 ...
 * 서버사이드 COPY) 로컬에서도 실제 버킷 없이 자리표시자 값으로 기동만 확인하지만, 사진은 서버가 원본을 내려받아 리사이즈해 다시 올려야 해서 로컬에서도
 * 실제로 검증할 S3 호환 서버(MinIO)가 필요하다 — 그래서 여기만 {@code endpoint}·{@code accessKey}·{@code secretKey}를
 * 갖는다. 두 추상화를 하나로 합칠지는 #213에서 별도로 정한다.
 *
