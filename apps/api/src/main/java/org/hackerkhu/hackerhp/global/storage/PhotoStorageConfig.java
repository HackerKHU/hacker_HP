 ...
 *
 * <p><b>자료(#207)의 {@code StorageConfig}가 만드는 {@code S3Client}·{@code S3Presigner}와 별개다.</b> 그쪽은 로컬에서도
 * MinIO 같은 엔드포인트 오버라이드를 지원하지 않는다 — 사진은 서버가 원본을 내려받아 리사이즈해야 해서 로컬에서도 실제로 검증할 S3 호환 서버가 필요하다
 * (자세한 이유는 {@link PhotoStorageProperties} 참고). 빈 이름이 겹치면 어느 쪽을 주입할지 알 수 없으므로 {@code @Qualifier}로
 * 갈라 둔다. 두 클라이언트를 하나로 합칠지는 #213에서 정한다.
 *
 * <p><b>운영은 정적 키를 쓰지 않는다</b> (docs/ops/infra.md MUST). {@code accessKey}·{@code secretKey}를 비워 두면 AWS
 * SDK 기본 자격 증명 체인이 ECS Task Role을 찾아 쓴다. 로컬(MinIO)만 {@code endpointOverride}와 정적 키가 필요하다 — MinIO는
 * IAM 역할이 없는 독립 서버라 그것 말고는 인증할 방법이 없다.
 */
@Configuration
