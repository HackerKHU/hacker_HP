...
@TestPropertySource(
    properties = {
      ...
      // 되는데, 대부분의 테스트는 FileStorage를 갈아끼우거나(자료, #53) 실제 S3를 아예
      // 부르지 않기 때문이다. 활동사진처럼 진짜 S3(MinIO)가 필요한 테스트는
      // PhotoApiIntegrationTest처럼 @DynamicPropertySource로 이 값을 덮어쓴다 (#213 통합 —
      // 자료·사진이 이제 같은 app.storage.bucket을 쓴다).
