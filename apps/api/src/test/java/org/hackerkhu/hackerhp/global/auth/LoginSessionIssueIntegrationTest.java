...
class LoginSessionIssueIntegrationTest extends AbstractIntegrationTest {
  ...
  /**
   * <b>T-202 — 이 이슈의 본체.</b> 로그인과 정지를 <b>동시에</b> 돌린다.
   *
   * <p>확인하는 것은 순서가 아니라 <b>어느 순서로 끝나든 세션과 DB가 같다</b>는 것이다. 예전에는 정지가 아직 저장되지 않은 세션을 놓쳐 <b>DB는 {@code
   * SUSPENDED}인데 세션은 {@code ACTIVE}로</b> 남았다 — 정지된 사람이 만료까지 계속 이용하는 상태다.
   *
   * <p>여러 번 돌리는 것은 두 순서를 모두 밟기 위해서다. 어느 쪽이 먼저 잠그든 결론은 하나여야 한다.
   */
  @RepeatedTest(5)
  void loginAndSuspensionAgreeWhicheverWinsTheLock() throws Exception {
    userRepository.deleteAll();
  ...
  /**
   * T-204 — 로그인 도중 계정이 사라지면 <b>세션을 만들지 않는다</b> (2-2 §2-2-4 MUST).
   *
   * <p>만들어 두면 계정 없는 사람이 만료까지 인증된다 — 필터는 매 요청 {@code users}를 읽지 않는다(3-3 결정 12). 승인·정지와 달리 <b>재로그인으로
   * 회복할 계정조차 없다.</b>
   *
   * <p>이 경우에는 <b>응답을 아직 내보내기 전</b>이라 로그인 실패로 되돌릴 수 있다.
   */
  @Test
  void issuesNothingWhenTheAccountIsGone() {
    Browser authorization = startAuthorization();
    MockHttpServletRequest request =
        new MockHttpServletRequest("GET", "/api/v1/login/oauth2/code/google");
    request.setCookies(authorization.cookie());
    ...
    run(
        ...
        response,
        (wrapped, wrappedResponse) -> {
          // 계정을 읽은 뒤, 세션이 만들어지기 전에 사라진 상황이다.
